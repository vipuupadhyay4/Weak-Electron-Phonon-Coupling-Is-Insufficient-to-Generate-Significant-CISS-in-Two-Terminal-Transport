...
for T in np.arange (0.05,0.35,0.01):
    ...
    for x in range(0,10):
        ...
        if cnthh==0:    
            ...
            for i in range(1, NE-1):
                z=EnG[i]+1j*eta1
                GR[i]=np.linalg.inv(z*np.eye(2 * Nf) - H - EP)
        ...
        with open("hilbert_errors4.txt", "w") as f:           
            ...
            # ===== Exact principal-value Hilbert =====
                ...
                print(f"grid{x}, Iteration {cnt}, Residual = {err1},Absolute={errA}")
                ...
                err2=err1.copy()
                np.save("GR_mid.npy", GR)
                np.save("GA_mid.npy", GA)
                np.save("GN_mid.npy", GN)
                np.save("GP_mid.npy", GP)
                cnthh=cnthh+1 
                cnt += 1
    ...
    # Append to Jnet
    Jnet1.append((T, Ilu, Ild,Ilp,IlT))
...
np.savetxt("Currents_with_Temperature_ml2_local.txt", real_data, fmt="%.9f")  # Adjust precision as needed



Jl = np.array(Jl)
x_vals = Jl[:, 0]  # Independent variable a
y1_vals = Jl[:, 1]  # Second function b
y2_vals = Jl[:, 2]  # Second function b
y3_vals = Jl[:, 3]  # Second function b
y4_vals = Jl[:, 4]  # Second function b

# Plot with thicker line and larger label
plt.plot(x_vals, y1_vals, linewidth=4,color='black')
plt.plot(x_vals, y2_vals, linewidth=4,color='green')
plt.plot(x_vals, y3_vals, linewidth=4,color='black')
plt.plot(x_vals, y4_vals, linewidth=4,color='green')
# Add labels with larger font
plt.xlabel(r'$E$', fontsize=18)
plt.ylabel('Currents', fontsize=18)
