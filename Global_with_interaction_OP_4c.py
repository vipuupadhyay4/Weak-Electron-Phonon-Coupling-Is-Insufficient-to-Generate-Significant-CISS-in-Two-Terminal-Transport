...
V=15*t0
cnt1=0
for ephi in np.arange (0.,0.14 ,0.01):

    t1 =ephi*t0
    lambda1 =ephi*lambda0
    ...
    for x in range(0,1):
        ...
        if cnt1 == 0:
            # FIRST ITERATION: analytic guess (always)
            ...
            # ALL SUBSEQUENT ITERATIONS: load previous solution
        ...
        with open("hilbert_errors.txt", "w") as f:           
            ...
            # ===== Exact principal-value Hilbert =====
                ...
                if cnt >= 4*(Nhis-1):
                    ...
                    GR_pulay = sum(coeff[i] * HGR[i] for i in range(Nhis))
    ...
    # Append to Jnet
    Jnet1.append((ephi, Ilu, Ild,Ilp,IlT))
    cnt1=cnt1+1
...
np.savetxt("Currents_with_ephi_op_n.txt", real_data, fmt="%.9f")  # Adjust precision as needed
