...
for V in np.arange (15*t0,16*t0,4*t0):
    ...
    for x in range(0,1):
        ...
        GR=np.load("GR_final.npy")
        GA = np.load("GA_final.npy")
        GN = np.load("GN_final.npy")        
        GP= np.load("GP_final.npy")
        ...
        while cnt <= 5000:
           ...
           # ===== Exact principal-value Hilbert =====
            ...
            err2=np.linalg.norm(R_new)

           
           
           
            print(f"Iteration {cnt}, Residual = {err1},Absolute={err2}")
...
np.savetxt("Currents_with_energy_Self_Consistent_Hilbert.txt", real_data, fmt="%.9f")  # Adjust precision as needed
